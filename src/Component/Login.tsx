    ...
    if (email && password) {
      try {
        const response = await axios.post(
          "https://school-management-school-system.e5fb6t.easypanel.host/api/login", 
          {
            email: email,
            password: password,
          },
        ...
        <CardContent className="">
          <form onSubmit={handlelogin}>
            ...
            <div className="flex items-center justify-between rounded-md px-2 border-[1px] border-[#D0D5DD]">
              ...
              />
              <p onClick={() => setshow(!Showpassword)}>
            ...
            <Input
              type="submit"
              className="bg-[#1A55A5] cursor-pointer hover:bg-[#428bf8] text-white mt-6 md:mt-[5vh]"
            />
            <p className="text-center text-red-600 mt-7 text-[12px]" >{error}</p>
          </form>
        </CardContent>
      </Card>
    </div>
